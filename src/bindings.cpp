...
PYBIND11_MODULE(_dynsys, m) {
    ...
    py::class_<SimulationCPU>(m, "SimulationCPU")
        ...
        .def(
            ...
            [](SimulationCPU& self, const size_t num_points, const IntegratorType iType,
               ...
               std::string varSysCode, const size_t numOfTransitionPoints) {
                auto data = self.createOneDimLyapunovDiagram(
                    num_points, iType, parameterIdx, numOfConstants, minValue, maxValue, deltaValue,
                    xyzOrder, varSysCode, numOfTransitionPoints);
        ...
        .def(
            ...
            [](SimulationCPU& self, const size_t num_points, const IntegratorType iType,
               ...
               const size_t numOfTransitionPoints) {
                auto data = self.createTwoDimLyapunovDiagram(
                    ...
                    maxValue1, deltaValue1, minValue2, maxValue2, deltaValue2, xyzOrder, varSysCode,
                    numOfTransitionPoints);
                size_t rows = (size_t)((maxValue2 - minValue2) / deltaValue2) + 1;
                size_t cols = (size_t)((maxValue1 - minValue1) / deltaValue1) + 1;
                return py::array(py::dtype::of<float>(), {rows, cols, static_cast<size_t>(1)},
            ...
            py::arg("var_sys_code"), py::arg("num_transition_points"),
            R"pbdoc(
