...
llvm::Expected<std::unique_ptr<llvm::Module>> compileToIR(std::string& codeString, llvm::LLVMContext& context) {
    ...
    compiler.getPreprocessorOpts().addRemappedFile("jit.c", llvm::MemoryBuffer::getMemBuffer(codeString).release());

    // TODO: Add optimization level

    clang::EmitLLVMOnlyAction irAction(&context);
    if (!compiler.ExecuteAction(irAction)) {
        return llvm::make_error<llvm::StringError>("Failed to create IR", llvm::inconvertibleErrorCode());
    }
...
int try_jit(std::string& code, integratedFunc& func, std::unique_ptr<llvm::orc::LLJIT>& jit) {
    ...
    llvm::orc::ThreadSafeModule tsm(std::move(irModule.get()), std::make_unique<llvm::LLVMContext>());
