...
class Model:
    ...
    def __init__(self, dt: float, initial: list[float], params: numpy.ndarray):
        ...
        self.num_constants = params.shape[0]
    ...
    def bifurcation(self, steps: int, integrator_type: str, num_transition_points: int, parameter_idx: int, point_component_idx: int, min_max_dt: tuple):
        assert steps > 0, "steps must be positive int"
        assert integrator_type in ['euler', 'rk4', 'midpoint', 'euler-cromer'], "invalid integrator type"
        ...
        return self.sim.createOneDimBifurcationDiagram(
            steps, select_integrator[integrator_type], parameter_idx, point_component_idx, self.num_constants,
            min_max_dt[0], min_max_dt[1], min_max_dt[2], num_transition_points
        )